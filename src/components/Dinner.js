...
<p>{item.type}</p>
${item.price}   
<button id={i} onClick={this.handleClick}  name={ item.type} value={item.price} type="button" className="btn btn-primary "><FontAwesomeIcon icon="plus-square" /></button>
</Card>
</div>
))}
...
<Table>
      ...
      <TableBody>
       ...
       <td width="200"  pr-1>{menu.type}</td>
        ...
        <td width="200" ><button type="button" className="btn btn-primary btn-sm "  onClick={() => this.handleDelete(i)}><FontAwesomeIcon icon="trash-alt" /></button></td>
        </tr>
 ...
 <tr>
          ...
          <td width="200" ><b>{this.state.total}</b></td>
          <td width="200" ></td>
        </tr>
        <tr>
          <td width="200" ></td>
